...
function partition(innerData) {
  ...
  const sumEd = innerRoot.sum(d => d.value);
...
(async function () {
  // const data = await fetch('data/sunburst.json').then(response => response.json());
  ...
  // const color = d3.scaleOrdinal(d3.quantize(d3.interpolateRainbow, data.length + 1)); // for sunburst.json
  ...
  const path = g.append("g")
    ...
    .join("path")
    .attr("fill", d => {
      while (d.depth) d = d.parent; return color(d.data.name);
    })
