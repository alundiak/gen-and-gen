// from https://www.jasondavies.com/coffee-wheel/
...
! function () {
    ...
    function n(t) {
        // Commented this to have only 2-colors based layout.
        // if (t.children) {
    ...
    var i = 900,
        l = i,
        o = i / 2,
        d = d3.scale.linear().range([0, 2 * Math.PI]), // D3 v3
        // d = d3.scaleLinear().range([0, 2 * Math.PI]), // D3 v4
        u = d3.scale.pow().exponent(1.3).domain([0, 1]).range([0, o]), // D3 v3
        // u = d3.scalePow().exponent(1.3).domain([0, 1]).range([0, o]), // D3 v4
        c = 5,
        s = 1e3,
        h = d3.select("#vis");
    ...
    var p = d3.layout.partition().sort(null).value(function (t) { // D3 v3
        // var p = d3.partition().sort(null).value(function (t) { // D3 v4 ?
        return 5.8 - t.depth
    }),
    ...
    // d3.json("./data/wheel.json", function(r, i) {
    d3.json("./data/sunburst.json", function (r, i) {
        function l(n) {
        ...
        y.append("title")
            .text(function (d) {
                return `${d.year} - ${d.firstName} ${d.name}`;
            });
    })
}();
